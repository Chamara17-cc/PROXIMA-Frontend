...
export default function TaskReport() {
    ...
    useEffect(() => {
      ...
      const fetchDigramData = async (developerId) => {
        try {
          const responseremaintask = await axios.get(`https://localhost:7008/api/DeveloperTaskProgress/GetRemainingTasks/${developerId}`);
          setRemainTask(responseremaintask.data);
          console.log(remainTask);

          const responsecompletetask = await axios.get(`https://localhost:7008/api/DeveloperTaskProgress/GetCompletedTasks/${developerId}`);
          setCompleteTask(responsecompletetask.data);
          console.log(completeTask);

          const responseinprogresstask = await axios.get(`https://localhost:7008/api/DeveloperTaskProgress/GetInProgressTasks/${developerId}`);
